...
| Deployed evidence | Task 09's provisioning, deployment and mock-only Discord smoke are historical records from 2026-09-14–17. Task 19's [dated read-only staging observation](architecture/configuration.md#task-19-read-only-staging-preflight--2026-09-19) independently reconfirmed the older mock/discovery-disabled Worker and D1 journal `0001`–`0004` on 2026-09-19; it did not deploy or activate anything. Task 13/18 runtime and migrations `0005`/`0006` remain **merged, not deployed**. |
...
| Task 20 controlled smoke | The [reviewed runbook](task20-staging-follow-smoke.md) and local approval bundle received one-run staging/mock approval. The [2026-09-19 attempt](architecture/configuration.md#task-20-approved-attempt-stopped-at-the-queue-baseline--2026-09-19) stopped at the initial read-only Queue topology gate: all three Queue GETs omitted `delivery_paused`. The single approved bot preflight later passed. A separate signed-in Dashboard read then showed all three named Queues explicitly Active; those dated readings are not a later checkpoint. No deployment, migration, publication or discovery activation occurred. Issue #35 remains blocked pending review of the replacement verification method and fresh checkpoint evidence; the bot preflight must not be repeated. |
...
| [architecture/data-model-and-outbox.md](architecture/data-model-and-outbox.md) | D1 schema, migrations, identifiers, transactional outbox, Task 13 migration `0005`, and Follow migration `0006`. |
