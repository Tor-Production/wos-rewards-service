...
| Task 20 preparation | The [controlled staging Follow smoke runbook](task20-staging-follow-smoke.md) records the isolated cutover, fresh read-only baseline, and approval gate. It has not migrated, deployed, logged in, published or enabled discovery. The known local Task 19 tuple/auth paths were absent at preparation, so the exact approval bundle is not frozen. |
