...
| Deployed evidence | Task 09's mock-only Discord smoke and Task 19's [read-only observation](architecture/configuration.md#task-19-read-only-staging-preflight--2026-09-19) are dated historical records. Task 21 later deployed the guarded staging runtime and applied migrations `0005`/`0006`, then restored discovery-disabled normal staging. Task 23's `0007` remains in unmerged PR #43 and is not deployed. |
...
| Community JSON source in PR #43 | Disabled staging/mock scheduled path with an exact endpoint, durable 30-minute request gate, baseline-only first snapshot, bounded reconciliation and immutable first observation (migration `0007`). It is not merged, migrated, deployed or activated. See [discovery policy §7](whiteout-provider-decision.md#7-gift-code-discovery-source-status). |
