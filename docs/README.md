...
| Task 20 controlled smoke | The [reviewed runbook](task20-staging-follow-smoke.md) and local approval bundle received one-run staging/mock approval. The [2026-09-19 attempt](architecture/configuration.md#task-20-approved-attempt-stopped-at-the-queue-baseline--2026-09-19) stopped at the initial read-only Queue topology gate: all three Queue GETs omitted the required `delivery_paused` boolean. The separately approved single bot preflight later passed, but no deployment, migration, publication or discovery activation occurred. Issue #35 is blocked pending a reliable pause-state gate and revised review. |
