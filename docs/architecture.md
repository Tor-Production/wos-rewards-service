...
| Layer | Key | Mechanism |
|---|---|---|
| Discord event acceptance | `processed_events.event_id` | marker inserted **only** in the same atomic `db.batch()` as the validation-reply delivery row (invalid) or the registration work + outbox rows (valid); after batch rollback, an existing marker identifies a duplicate; Phase 3 captures the complete code snapshot and writes `work_committed` directly, with an atomic 2,000-code cap ([acceptance details](architecture/discord-ingestion-and-registration.md#atomic-acceptance)) |
