...
| `COMMUNITY_JSON_SOURCE_ENABLED` | disabled community JSON adapter only | defaults to `false`; may be true only alongside enabled discovery in staging/mock, but Task 23 adds no scheduler, route, deployment value, or activation |
