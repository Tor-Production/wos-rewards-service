...
| Execution lease expires during a potentially applying provider call | Outcome may be unknown | The Task 13 dispatch hold denies T2/T12 replay; **T17** retains the hold and records effective uncertainty for verification. Only the actual network-free mock is unheld and may use the prior T2/T12 recovery path. |
| Invocation crashes after a potentially applying dispatch | Outcome may be unknown | On expiry the held row follows **T17** to effective uncertainty, without another provider call. An unheld safe-mock invocation may be reacquired via T2 or reset via T12. |
...
| Provider redeems then Worker crashes before recording | Ambiguous redemption | The Task 13 dispatch hold survives; expiry follows **T17**, closes local accounting as uncertainty, and forbids runtime replay or generic repair. Authorized upstream reconciliation or contract-backed idempotency is still required to resolve the outcome; production redemption remains blocked. |
...
| Invocation crash | A held potentially applying invocation expires → **T17** effective uncertainty with no replay. Only an unheld safe-mock invocation may be reacquired through **T2** or reset through **T12**. |
| Execution lease expires during a potentially applying provider call | The dispatch hold prevents a redelivery from stealing the call. Recovery records **T17** uncertainty and preserves the hold; late results may settle only before that finalization. Provider timeout must still remain below the lease. |
