...
The maintainer's [Channels Followed screenshot](https://share.zight.com/01a0b7f2-c3d4-7d9f-9799-3d50a0a43671) shows `3604 [LOS] #codes-source-test` posting into `#wos-code-feed` alongside a separate official Follow. It does not reveal the controlled source guild ID or the Follow-generated webhook ID. The staging bot has no source-channel permission; it observes only the destination feed. Do not substitute the other Follow, infer an ID from the displayed name, or loosen the exact source/webhook filter. Discord documents that [Channel Following adds a webhook in the destination server](https://support.discord.com/hc/en-us/articles/360028384531-Channel-Following-FAQ). Resolve the tuple from a previously delivered controlled destination message or an approved fresh Gateway envelope, with the required bot credentials kept private.
...
$tuplePath = '<absolute path to restored, ignored non-secret tuple>'
