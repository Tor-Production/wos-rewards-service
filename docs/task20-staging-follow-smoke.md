...
2. **Before any Cloudflare mutation**, run one bounded bot preflight with the existing private file and the two selected historical message IDs from the ignored approval bundle. It opens a bot Gateway session for at most 20 seconds, checks the staging application identity, makes one exact destination-message GET with a 10-second timeout, compares its Follow metadata to the frozen tuple, then destroys the client. This separate entrypoint has no `MESSAGE_CREATE` listener and never calls the Worker. It logs no token, message body or IDs. A 403, rate limit, mismatch, timeout or login failure is a hard stop; do not retry, rotate credentials or proceed to queue changes. The historical webhook-object GET is not repeated. This check is live and remains unexecuted until approval.
