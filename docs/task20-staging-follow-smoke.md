...
Status: **historical reviewed candidate; approved attempt stopped at the initial read-only gate**. Owner: [issue #35](https://github.com/Tor-Production/wos-rewards-service/issues/35); partial parent [#22](https://github.com/Tor-Production/wos-rewards-service/issues/22). This page preserves the reviewed command sequence and its frozen input requirements. The ignored four-field tuple matches Task 19's earlier exact-message metadata, including its observed `webhook_id`; the private staging file supplies the two required companion names. The historical exact webhook-object GET and newer exact-message/application GETs returned 403 for different reads. The one-run approval did not produce a cutover or a live smoke. **Do not execute this historical sequence again without a revised reviewed proposal and fresh approval.**

**Dated execution note — 2026-09-19:** The maintainer approved one controlled staging/mock run, but its first read-only Queue topology check stopped before any mutation. All three Queue GET responses omitted the `settings.delivery_paused` boolean, so the required unpaused baseline could not be proven. The separately approved single bot preflight later passed, then the run stopped with the Queue gate still unresolved. The reviewed candidate was not deployed, and no announcement was published. See the [dated evidence](architecture/configuration.md#task-20-approved-attempt-stopped-at-the-queue-baseline--2026-09-19). This preparation sequence is preserved as the reviewed historical candidate; another attempt needs a reliable pause-state gate, a revised reviewed candidate and fresh approval.
...
Queue pause/resume, deployments, migrations, Discord login/publication and intake are **live external actions**. Only the bounded bot preflight login occurred in the 2026-09-19 stopped attempt; no foreground companion login, publication, intake or Cloudflare mutation occurred. That run's approval does not authorize a later retry of this historical sequence.
