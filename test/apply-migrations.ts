import { applyD1Migrations } from "cloudflare:test";
import { env } from "cloudflare:workers";

/**
 * Apply the baseline schema to the test database.
 *
 * Storage isolation is per test file, so setup can run several times in a session.
 * `applyD1Migrations()` only applies migrations that are not already recorded in the
 * `d1_migrations` table, so calling it repeatedly is a no-op — the migrated schema is the
 * baseline shared by tests in each isolated file.
 *
 * This targets the local Miniflare D1 database behind the `STAGING_DB` binding. No remote
 * database is contacted and no Cloudflare credentials are used.
 */
await applyD1Migrations(env.STAGING_DB, env.TEST_MIGRATIONS);

await applyD1Migrations(env.BASELINE_DB, env.TEST_MIGRATIONS.slice(0, 1));

await applyD1Migrations(env.PHASE4_DB, env.TEST_MIGRATIONS);
await applyD1Migrations(env.THROUGHPUT_DB, env.TEST_MIGRATIONS);
